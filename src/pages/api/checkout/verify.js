...
function createOrderDataFromPaymentIntent(paymentIntent) {
  ...
  return {
    ...
    items: [{
      product_id: metadata.first_item_id || 'unknown',
      product_name: metadata.first_item_name || 'Travel Data WiFi Product',
...
async function createFallbackOrder(paymentIntent) {
  ...
  try {
    ...
    console.log('⚠️ Fallback order creation not implemented yet');
    return orderData;
