...
export default CheckoutSuccessPage;// ===== src/pages/checkout/success.tsx =====
import React, { useState, useEffect } from 'react';
import { useRouter } from 'next/router';
import Layout from '../../components/Layout';
import { useCartStore } from '../../store/cart';
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        // Fetch order details from Zoho or your API
        const response = await fetch('/api/checkout/verify', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      <div className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl font-bold text-gray-900 mb-4">Order Confirmed!</h1>
            <p className="text-xl text-gray-600 max-w-2xl mx-auto">
              Thank you for your purchase! Your order has been successfully processed through Zoho Commerce.
            </p>
          </div>
              ...
              <div className="bg-white rounded-lg shadow-sm p-8 mb-8">
                ...
                <div className="grid grid-cols-2 gap-6 mb-8">
                  <div>
                    <h3 className="text-sm font-medium text-gray-700 mb-2">Order Number</h3>
                    <p className="text-lg font-semibold text-gray-900">
                      {orderData?.orderNumber || orderData?.order_id || 'TDW-' + (session_id || order_id)}
                    </p>
                  </div>
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-700 mb-2">Payment Method</h3>
                    <p className="text-gray-900">
                      {orderData?.paymentMethod || 'Credit Card'} •••• {orderData?.last4 || '****'}
                    </p>
                  </div>
