// src/lib/redis-product-service.js
...
export class RedisProductService {
  ...
  async syncProducts() {
    try {
      ...
      // Set sync status
      await redis.setex(RedisProductService.KEYS.SYNC_STATUS, 300, JSON.stringify({
        status: 'syncing',
      ...
      // Store all products
      promises.push(redis.setex(RedisProductService.KEYS.ALL_PRODUCTS, RedisProductService.TTL, JSON.stringify(activeProducts)))

      // Store individual products for fast lookups
      activeProducts.forEach(product => {
        promises.push(redis.setex(
      ...
      // Store sync metadata
      promises.push(redis.setex(RedisProductService.KEYS.LAST_SYNC, RedisProductService.TTL, Date.now()))

      // Update sync status
      promises.push(redis.setex(RedisProductService.KEYS.SYNC_STATUS, 300, JSON.stringify({
        status: 'completed',
    ...
    for (const product of products) {
      if (product.image_id) {
        ...
        const imageData = {
          ...
          proxy_url: `/api/images/${product.item_id}`,
          cached_at: Date.now()
        }
  ...
  async getCacheStats() {
    try {
      ...
      const products = productCount ? (typeof productCount === 'string' ? JSON.parse(productCount) : productCount) : []
      const status = syncStatus ? (typeof syncStatus === 'string' ? JSON.parse(syncStatus) : syncStatus) : null
  ...
  filterProductsByDisplayInApp(inventoryProducts) {
    return inventoryProducts.filter(product => {
      ...
      return displayField.value === true ||
